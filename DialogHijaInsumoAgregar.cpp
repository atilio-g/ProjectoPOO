...
/// CONSTRUCTOR
DialogHijaInsumoAgregar::DialogHijaInsumoAgregar(wxWindow *parent, GestorPanaderia *gestor) : 
	DialogBaseInsumo(parent), m_gestor(gestor) {
...
DialogHijaInsumoAgregar::~DialogHijaInsumoAgregar() {
	
}

/// Actualizo texto costo / unidad
void DialogHijaInsumoAgregar::OnEligeUnidad( wxCommandEvent& event )  
{
...
/// GUARDAR INSUMO
void DialogHijaInsumoAgregar::OnClickGuardar( wxCommandEvent& event )  
{
	...
	if ( errores.size() == 0 )
	{
		if ( not m_gestor->ValidarNombreInsumo(nombre) )
	...
	// no hay errores: agrego el insumo y guardo
...
void DialogHijaInsumoAgregar::OnClickCancelar( wxCommandEvent& event )  
{
	this->EndModal(0);
}
